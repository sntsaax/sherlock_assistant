...
st.write("---")
st.write(
    "Welcome, Detective. Use the system to analyze case evidence files strictly and securely."
)
...
with st.sidebar:
    ...
    # Navigation Menu
    page = st.radio(
        "Navigation Menu", ["Home Page", "List of Cases", "Quick Case Submission"]
    )
...
if page == "Home Page":
    ...
    if cases is None:
        ...
        st.warning(
            "No active case files loaded. Go to 'Quick Case Submission' to upload evidence first."
        )
    else:
        # Context binding selection
        options_map = {f"{c['filename']} (ID: {c['id'][:8]})": c["id"] for c in cases}
        selected_label = st.selectbox(
            "Select Active Case Context:", list(options_map.keys())
        )
    ...
    if cases is None:
        ...
        st.info(
            "System memory workspace inventory is currently empty. Go upload an evidence file!"
        )
    else:
    ...
    with st.form("upload_form", clear_on_submit=True):
        subject_input = st.text_input(
            "Case Subject / Description:", placeholder="e.g., Alibi Verification"
        )
        ...
        if submit_btn:
            if not subject_input or not uploaded_file:
                st.error(
                    "Please provide both a subject description and an evidence file."
                )
            else:
                with st.spinner("Processing file and running RAG engine ingestion..."):
                    try:
                        ...
                        if response.status_code == 201:
                            st.success(
                                f"Success! '{uploaded_file.name}' has been safely ingested into ChromaDB."
                            )
                        else:
                        ...
                        st.error(
                            f"Could not connect to FastAPI backend server: {str(e)}"
                        )
