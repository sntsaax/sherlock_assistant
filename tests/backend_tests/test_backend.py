...
class TestBackendIngestion(unittest.TestCase):
    ...
    def test_fr_2_3_query_routing_success(self, mock_sherlock_answer):
        ...
        mock_sherlock_answer.return_value = (
            "The suspect was confirmed to be at the club during the incident."
        )

        response = self.client.post(
            "/query", json={"question": "Where was the suspect?", "case_id": active_id}
        )
        ...
        mock_sherlock_answer.assert_called_once_with(
            "Where was the suspect?", active_id
        )

    def test_fr_2_3_query_routing_missing_case(self):
        """FR-2.3: Verify querying a non-existent case_id returns a 404 Error."""
        response = self.client.post(
            "/query", json={"question": "Any updates?", "case_id": "ghost-id-999"}
        )
        self.assertEqual(response.status_code, 404)
