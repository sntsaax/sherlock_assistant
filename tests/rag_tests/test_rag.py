...
class Test_FR_3_1(unittest.TestCase):
    def test_text_extraction_and_chunking(self):
        ...
        with open(filename, "w", encoding="utf-8") as f:
            f.write(
                "This is a sample witness statement. It needs to be long enough to chunk."
            )
    ...
    def test_similarity_search_and_retrieval(self):
        ...
        try:
            ...
            retrieved_chunks = query_vector_store(
                "Where is the missing diamond?", case_id, n_results=1
            )
...
class Test_FR_3_4(unittest.TestCase):
    def test_context_augmented_prompt_generation(self):
        ...
        with open(filename, "w", encoding="utf-8") as f:
            f.write(
                "Evidence item: The footprint found in the mud belongs to a size 11 Goodyear boot."
            )
        ...
        try:
            ...
            final_prompt = generate_augmented_prompt(
                "What size was the boot footprint?", case_id
            )

            self.assertIn(
                "You are Sherlock, an expert investigative digital assistant.",
                final_prompt,
            )
...
class Test_FR_3_5(unittest.TestCase):
    def test_llm_answer_generation(self):
        ...
        if not os.getenv("OPENAI_API_KEY"):
            self.skipTest(
                "Skipping FR-3.5 because OPENAI_API_KEY is not set in environment or .env file"
            )
        ...
        with open(filename, "w", encoding="utf-8") as f:
            f.write(
                "Evidence item: The broken window latches indicate entry was forced from the outside."
            )
